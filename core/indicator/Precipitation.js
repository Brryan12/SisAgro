...
function process(dataset_, filter) {
  ...
  var dataset = dataset_.select(["pr"], ["p"]);
...
var Component = {
  layer: function (filter, dataset) {
    var data = dataset.mean().clip(filter.geometry).select("p");
    return Response.success(data);
  },
  number: function (filter, dataset) {
    var dataMean = Utils.monthData(dataset, filter.yearRange.get("end"), Utils.datasetMean)
      .toBands()
      .reduceRegion(ee.Reducer.mean(), filter.geometry, filter.indicator.scale);
    var data = Utils.processMonthData(dataMean, { p: 0 }, Utils.mean);
    return Response.success(data);
  },
  chart: function (filter, dataset) {
    var data = {
      dataset: dataset.select(["p"], ["Precipitación"]),
      reducer: "mean"
    };
    return Response.success(data);
  }
